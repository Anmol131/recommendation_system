...
const FALLBACK_IMAGE = 'https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=1600';
  ...
  return (
    <div className="min-h-screen bg-surface px-6 py-20 text-on-surface">
      <div className="mx-auto max-w-2xl rounded-2xl bg-surface-container-low p-8 text-center shadow-[0_25px_55px_-26px_rgba(62,37,72,0.28)]">
        <h1 className="text-3xl font-bold tracking-tight">Track not available right now</h1>
        <p className="mt-3 text-on-surface-variant">
          We could not load this track at the moment. Try going back and opening another song.
        </p>
        <button
          type="button"
          onClick={() => navigate(-1)}
...
function MusicDetailPage() {
    ...
    const loadPage = async () => {
      ...
      try {
        const [detail, similar] = await Promise.all([
          endpoints.getMusicByTrackId(lookupId),
          endpoints.getMusic({ limit: 4, sort: 'popularity' }),
        ]);

        if (cancelled) return;

        const track = detail?.data || detail;
        const currentId = track?.trackId || track?._id;
        const similarList = getList(similar)
          .filter((entry) => String(entry?.trackId || entry?._id) !== String(currentId))
          .slice(0, 4);
  ...
  useEffect(() => {
    ...
    endpoints.addHistory('music', lookupId, 'viewed').catch(() => {});
  }, [lookupId, isAuthenticated]);

  const genres = useMemo(() => normalizeGenres(item?.genre || item?.genres), [item]);
      ...
      <nav className="sticky top-0 z-50 w-full bg-white/60 backdrop-blur-md shadow-[0_20px_40px_-10px_rgba(62,37,72,0.08)]">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-8 text-sm font-medium tracking-wide">
          ...
          <div className="hidden items-center gap-8 md:flex">
            ...
            <button type="button" onClick={() => navigate('/music')} className="font-semibold text-primary">Music</button>
          </div>
            ...
            <div className="max-w-3xl space-y-6">
              <div className="flex flex-wrap items-center gap-4">
                <span className="rounded-full bg-secondary-container px-3 py-1 text-xs font-bold uppercase tracking-widest text-on-secondary-container">
                  {item.genre || genres[0] || 'Music'}
                </span>
                <div className="flex items-center gap-1 text-tertiary-fixed-dim">
                  <Star size={14} className="fill-current" />
                  <span className="text-sm font-bold">{formatPopularity(item.popularity)} Popularity</span>
                </div>
                <span className="text-sm font-medium text-white/80">
                  {item.durationSec ? `${item.durationSec}s` : 'Duration N/A'}
                </span>
              </div>
              ...
              <div>
                ...
                <p className="mt-3 text-xl text-white/85">{item.artist || 'Unknown Artist'}</p>
              </div>

              <p className="max-w-xl text-lg font-medium leading-relaxed text-white/90">
                {item.album || 'Album not available'} • {item.artist || 'Unknown artist'} • {item.genre || 'Unknown genre'}
              </p>
        ...
        <section className="mx-auto grid max-w-[1440px] grid-cols-1 gap-16 px-8 py-24 md:grid-cols-12">
          <div className="space-y-12 md:col-span-8">
            ...
            <div className="grid grid-cols-1 gap-6 pt-4 sm:grid-cols-3">
              <div className="space-y-2 rounded-xl bg-surface-container-low p-6 shadow-[0_16px_35px_-24px_rgba(62,37,72,0.3)]">
                <span className="text-xs font-bold uppercase tracking-widest text-primary/60">Artist</span>
                <p className="text-lg font-bold">{item.artist || 'Unknown'}</p>
              </div>
              <div className="space-y-2 rounded-xl bg-surface-container-low p-6 shadow-[0_16px_35px_-24px_rgba(62,37,72,0.3)]">
                <span className="text-xs font-bold uppercase tracking-widest text-primary/60">Album</span>
                <p className="text-lg font-bold">{item.album || 'Not listed'}</p>
              </div>
              <div className="space-y-2 rounded-xl bg-surface-container-low p-6 shadow-[0_16px_35px_-24px_rgba(62,37,72,0.3)]">
                <span className="text-xs font-bold uppercase tracking-widest text-primary/60">Genre</span>
                <p className="text-lg font-bold">{item.genre || genres[0] || 'N/A'}</p>
              </div>
            </div>
          </div>
          ...
          <aside className="space-y-8 md:col-span-4">
            <div className="space-y-6 rounded-2xl bg-surface-container-highest p-8 shadow-[0_20px_45px_-28px_rgba(62,37,72,0.4)]">
              <h3 className="text-xl font-bold">Listen On</h3>
              <div className="space-y-4">
                <a
                  href={item.spotifyUrl || 'https://open.spotify.com'}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                >
                  <span className="font-semibold transition-colors group-hover:text-primary">Spotify</span>
                  <ExternalLink size={14} className="text-on-surface-variant" />
                </a>
                <a
                  href={item.lastfmUrl || 'https://www.last.fm'}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-center justify-between rounded-lg bg-surface-container-lowest p-4 shadow-[0_12px_24px_-20px_rgba(62,37,72,0.35)]"
                >
                  <span className="font-semibold transition-colors group-hover:text-primary">Last.fm</span>
                  <ExternalLink size={14} className="text-on-surface-variant" />
                </a>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {genres.length > 0 ? genres.map((genre) => (
                  ...
                  <span className="rounded-full bg-surface-container-low px-3 py-1 text-xs font-medium">Music</span>
                )}
              </div>
            </div>
          </aside>
          ...
          <div className="mx-auto max-w-[1440px] px-8">
            <div className="mb-12 flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <p className="font-medium text-on-surface-variant">Curated based on your current listening mood.</p>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
              {similarItems.map((similar) => (
                <button
                  key={similar.trackId || similar._id}
                  type="button"
                  onClick={() => navigate(`/music/${similar.trackId || similar._id}`)}
                ...
                >
                  <div className="relative mb-4 aspect-[2/3] overflow-hidden rounded-xl shadow-lg transition-all duration-300 group-hover:scale-[1.02] group-hover:shadow-2xl">
                    ...
                    <div className="absolute right-4 top-4 rounded-md bg-surface/80 px-2 py-1 text-[10px] font-bold backdrop-blur-md">
                      {formatPopularity(similar.popularity)}
                    </div>
                  </div>
                  <h3 className="text-lg font-bold transition-colors group-hover:text-primary">{similar.title}</h3>
                  <p className="text-sm text-on-surface-variant">{similar.artist || 'Unknown Artist'} • {(similar.genre || 'Track')}</p>
                </button>
              ))}
            </div>
          </div>
