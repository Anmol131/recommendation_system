...
const FALLBACK_IMAGE = 'https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=1600';
...
const formatRuntime = (item) => {
  ...
  if (item?.durationMin) return `${item.durationMin}m`;
  return 'Runtime N/A';
};
  ...
  return (
    <div className="min-h-screen bg-surface px-6 py-20 text-on-surface">
      <div className="mx-auto max-w-2xl rounded-2xl bg-surface-container-low p-8 text-center shadow-[0_25px_55px_-26px_rgba(62,37,72,0.28)]">
        <h1 className="text-3xl font-bold tracking-tight">Movie not available right now</h1>
        <p className="mt-3 text-on-surface-variant">
          We could not load this title at the moment. Try going back and opening another movie.
        </p>
        <button
          type="button"
          onClick={() => navigate(-1)}
...
function MovieDetailPage() {
    ...
    const loadPage = async () => {
      ...
      try {
        const [detail, similar] = await Promise.all([
          ...
          endpoints.getMovies({ limit: 4, sort: 'rating' }),
        ]);

        if (cancelled) return;

        const movie = detail?.data || detail;
        const similarList = getList(similar)
          .filter((entry) => String(entry?._id) !== String(movie?._id))
          .slice(0, 4);
  ...
  useEffect(() => {
    ...
    endpoints.addHistory('movie', id, 'viewed').catch(() => {});
  }, [id, isAuthenticated]);
      ...
      <main>
        <section className="relative h-[86vh] min-h-[680px] w-full overflow-hidden bg-surface-container-lowest">
          <div className="absolute inset-0 z-0">
            <img src={item.poster || FALLBACK_IMAGE} alt={item.title} className="h-full w-full object-cover" />
          ...
          <div className="relative z-10 mx-auto flex h-full max-w-[1440px] flex-col justify-end px-8 pb-24">
            <div className="max-w-3xl space-y-6">
              <div className="flex flex-wrap items-center gap-4">
                ...
                <span className="text-sm font-medium text-white/80">{formatRuntime(item)}</span>
              </div>
              ...
              <p className="max-w-xl text-lg font-medium leading-relaxed text-white/90">
                {item.description || 'No storyline is available for this movie yet.'}
              </p>
          ...
          <div className="space-y-12 md:col-span-8">
            <div>
              ...
              <p className="text-xl leading-relaxed text-on-surface-variant">
                {item.description || 'No description is available for this movie yet.'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 pt-4 sm:grid-cols-3">
              <div className="space-y-2 rounded-xl bg-surface-container-low p-6 shadow-[0_16px_35px_-24px_rgba(62,37,72,0.3)]">
                <span className="text-xs font-bold uppercase tracking-widest text-primary/60">Director</span>
                <p className="text-lg font-bold">{item.director || 'Unknown'}</p>
              </div>
              <div className="space-y-2 rounded-xl bg-surface-container-low p-6 shadow-[0_16px_35px_-24px_rgba(62,37,72,0.3)]">
                <span className="text-xs font-bold uppercase tracking-widest text-primary/60">Cast</span>
                <p className="text-lg font-bold">{item.cast || 'Not listed'}</p>
              </div>
              <div className="space-y-2 rounded-xl bg-surface-container-low p-6 shadow-[0_16px_35px_-24px_rgba(62,37,72,0.3)]">
          ...
          <aside className="space-y-8 md:col-span-4">
            <div className="space-y-6 rounded-2xl bg-surface-container-highest p-8 shadow-[0_20px_45px_-28px_rgba(62,37,72,0.4)]">
              ...
              <div className="space-y-4">
                ...
                >
                  <span className="font-semibold transition-colors group-hover:text-primary">Netflix</span>
                ...
                <a
                  ...
                  className="group flex items-center justify-between rounded-lg bg-surface-container-lowest p-4 shadow-[0_12px_24px_-20px_rgba(62,37,72,0.35)]"
                >
                  <span className="font-semibold transition-colors group-hover:text-primary">Prime Video</span>
                  <ExternalLink size={14} className="text-on-surface-variant" />
                </a>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {genres.length > 0 ? genres.map((genre) => (
                  ...
                  <span className="rounded-full bg-surface-container-low px-3 py-1 text-xs font-medium">Featured</span>
                )}
              </div>
            </div>
          </aside>
          ...
          <div className="mx-auto max-w-[1440px] px-8">
            <div className="mb-12 flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <p className="font-medium text-on-surface-variant">Curated based on your current watch vibe.</p>
              </div>
              <button
                type="button"
              ...
              {similarItems.map((similar) => (
                ...
                >
                  <div className="relative mb-4 aspect-[2/3] overflow-hidden rounded-xl shadow-lg transition-all duration-300 group-hover:scale-[1.02] group-hover:shadow-2xl">
                    <img src={similar.poster || FALLBACK_IMAGE} alt={similar.title} className="h-full w-full object-cover" />
                  ...
                  <p className="text-sm text-on-surface-variant">{similar.year || 'N/A'} • {(normalizeGenres(similar.genres)[0] || 'Movie')}</p>
                </button>
              ))}
            </div>
          </div>
