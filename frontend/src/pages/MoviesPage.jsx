    ...
    <div className="min-h-screen bg-light-bg dark:bg-dark-bg pb-20 text-light-text dark:text-dark-text font-['Inter'] antialiased transition-colors duration-300">
      <main className="mx-auto max-w-7xl px-6 pb-16 pt-10 sm:px-8 lg:px-10">
        <div className="mb-10 flex justify-center">
          <nav className="flex items-center gap-1 rounded-lg bg-white dark:bg-slate-900 border border-surface-container p-1.5 shadow-md dark:shadow-dark-md">
        ...
        <header className="mb-10">
          <h1 className="text-5xl font-bold tracking-tight text-on-surface dark:text-white">Movies</h1>
          <p className="mt-3 max-w-2xl text-on-surface/70 dark:text-white/70">
            Explore top-rated picks from your catalog and open any title for full details.
          </p>
          <p className="mt-2 text-sm font-medium text-on-surface/60 dark:text-white/60">{total} titles loaded</p>
        </header>
        ...
        {!loading && error ? (
          <div className="rounded-2xl bg-surface-container-low p-8 text-center shadow-[0_20px_40px_-20px_rgba(62,37,72,0.3)]">
            ...
            <p className="mt-2 text-light-text dark:text-dark-text/95">Please try again in a moment.</p>
          </div>
        ) : null}
                ...
                >
                  ...
                  <div className="space-y-4 p-5">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-light-text dark:text-dark-text/95">
                        {(item?.year || 'Year N/A')} â€¢ {item?.genres?.[0] || item?.genre || 'Movie'}
                      </p>
                    </div>

                    <p className="line-clamp-3 text-sm leading-relaxed text-light-text dark:text-dark-text/95">
