const mongoose = require('mongoose');

const MONGO_URI =
	process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/recommendation_platform';

const DB_NAME = 'recommendation_platform';
const SOURCE_COLLECTION = 'movies';
const BACKUP_COLLECTION = 'movies_backup_before_normalize';

function toNumber(value) {
	if (value === null || value === undefined || value === '') return null;
	const num = Number(value);
	return Number.isNaN(num) ? null : num;
}

function toTrimmedString(value) {
	if (value === null || value === undefined) return null;
	const str = String(value).trim();
	return str === '' ? null : str;
}

function toBoolean(value, fallback = false) {
	if (typeof value === 'boolean') return value;
	if (typeof value === 'string') {
		const lower = value.trim().toLowerCase();
		if (lower === 'true') return true;
		if (lower === 'false') return false;
	}
	return fallback;
}

function normalizeGenres(value) {
	if (!value) return [];

	if (Array.isArray(value)) {
		return value
			.map((g) => String(g).trim())
			.filter(Boolean);
	}

	if (typeof value === 'string') {
		return value
			.split('|')
			.map((g) => g.trim())
			.filter(Boolean);
	}

	return [];
}

function normalizeCast(value) {
	if (!value) return null;

	if (Array.isArray(value)) {
		const cleaned = value.map((x) => String(x).trim()).filter(Boolean);
		return cleaned.length ? cleaned : null;
	}

	if (typeof value === 'string') {
		const cleaned = value
			.split(/[|,;]+/)
			.map((x) => x.trim())
			.filter(Boolean);
		return cleaned.length ? cleaned : null;
	}

	return null;
}

async function backupCollection(db) {
	const existing = await db.listCollections({ name: BACKUP_COLLECTION }).toArray();

	if (existing.length > 0) {
		console.log(`Backup collection "${BACKUP_COLLECTION}" already exists. Skipping backup.`);
		return;
	}

	console.log(`Creating backup collection: ${BACKUP_COLLECTION}`);
	await db.collection(SOURCE_COLLECTION).aggregate([{ $match: {} }, { $out: BACKUP_COLLECTION }]).toArray();
	console.log('Backup created successfully.\n');
}

async function normalizeMovies() {
	await mongoose.connect(MONGO_URI, { dbName: DB_NAME });
	const db = mongoose.connection.db;
	const movies = db.collection(SOURCE_COLLECTION);

	try {
		await backupCollection(db);

		const total = await movies.countDocuments({});
		console.log(`Normalizing ${total} movie documents...\n`);

		const cursor = movies.find({});
		const bulkOps = [];
		let processed = 0;

		while (await cursor.hasNext()) {
			const doc = await cursor.next();

			const normalized = {
				movieId: toNumber(doc.movieId),
				title: toTrimmedString(doc.title),
				year: toNumber(doc.year),
				genres: normalizeGenres(doc.genres),
				avgRating: toNumber(doc.avgRating),
				ratingCount: toNumber(doc.ratingCount),
				tmdbId: toNumber(doc.tmdbId),
				imdbId: toNumber(doc.imdbId),

				poster: toTrimmedString(doc.poster) ?? null,
				description: toTrimmedString(doc.description) ?? null,
				cast: normalizeCast(doc.cast),
				trailer: toTrimmedString(doc.trailer) ?? null,

				mediaType: doc.mediaType === 'tv' ? 'tv' : 'movie',
				enriched: toBoolean(doc.enriched, false),
			};

			bulkOps.push({
				updateOne: {
					filter: { _id: doc._id },
					update: { $set: normalized },
				},
			});

			processed += 1;

			if (bulkOps.length === 500) {
				await movies.bulkWrite(bulkOps);
				bulkOps.length = 0;
				console.log(`Processed ${processed}/${total}`);
			}
		}

		if (bulkOps.length) {
			await movies.bulkWrite(bulkOps);
		}

		console.log(`\nDone. Normalized ${processed} movie documents.`);
		console.log(`Backup collection kept as: ${BACKUP_COLLECTION}`);
	} catch (error) {
		console.error('Normalization failed:', error);
	} finally {
		await mongoose.disconnect();
	}
}

normalizeMovies();

