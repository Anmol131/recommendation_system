...
class GameRecommender:
    ...
    def _load_documents(self) -> None:
        logger.info("Loading game documents")
        collection = get_collection("games")
        raw_docs = list(collection.find({}))

        if not raw_docs:
            logger.warning("No game documents found")
            self.loaded = True
            return
        ...
        self.loaded = True
        logger.info("Game recommender loaded with %s items", len(self.items))
...
_game_recommender_instance = None


def get_game_recommender() -> GameRecommender:
    global _game_recommender_instance

    if _game_recommender_instance is None:
        _game_recommender_instance = GameRecommender()

    return _game_recommender_instance