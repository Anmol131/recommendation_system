...
class MusicRecommender:
    ...
    def _load_documents(self) -> None:
        logger.info("Loading music documents")
        collection = get_collection("musics")
        raw_docs = list(collection.find({}))

        if not raw_docs:
            logger.warning("No music documents found")
            self.loaded = True
            return
        ...
        self.loaded = True
        logger.info("Music recommender loaded with %s items", len(self.items))
...
_music_recommender_instance = None


def get_music_recommender() -> MusicRecommender:
    global _music_recommender_instance

    if _music_recommender_instance is None:
        _music_recommender_instance = MusicRecommender()

    return _music_recommender_instance