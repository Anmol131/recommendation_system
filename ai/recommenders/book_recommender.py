...
class BookRecommender:
    ...
    def _load_documents(self) -> None:
        logger.info("Loading book documents")
        collection = get_collection("books")
        raw_docs = list(collection.find({}))

        if not raw_docs:
            logger.warning("No book documents found")
            self.loaded = True
            return
        ...
        self.loaded = True
        logger.info("Book recommender loaded with %s items", len(self.items))
...
_book_recommender_instance = None


def get_book_recommender() -> BookRecommender:
    global _book_recommender_instance

    if _book_recommender_instance is None:
        _book_recommender_instance = BookRecommender()

    return _book_recommender_instance